...
export const useDouYinPublishStore = defineStore(
  'douYinPublish',
  () => {
    ...
    const toRequestParam = (
      userId: number | string,
      form: DouYinPublishForm
    ): DouYinAccountRelation => {
      return {
        id: `${userId}`,
        title: form.title,
        ...
        releaseTime:
          form.releaseType === 0
            ? 0
            : date
              .extractDate(form.releaseDateString, 'YYYY-MM-DD HH:mm')
              .getTime()
      }
    }
    const currentBatchConfig = computed({
      get () {
        return (
          batchConfigMap.value[route.params.id as string] ?? getDefaultConfig()
        )
      },
      set (newValue) {
